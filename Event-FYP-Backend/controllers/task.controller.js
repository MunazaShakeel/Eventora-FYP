...
exports.createTask = async (req, res) => {
    try {
        ...
        try {
            const studentId = volunteerRegistration.student_id;
            await sendNotification(
                studentId,
                'New Task Assigned',
                `You have been assigned a new task: "${title}" for the event "${event.title}".`,
                'task',
                task._id
            );
        } catch (notifyErr) {
            console.error('Notification error (createTask):', notifyErr.message);
        }
...
exports.updateTaskStatus = async (req, res) => {
    try {
        ...
        // 🆕 Notify Organizer when task is completed
            ...
            try {
                ...
                if (event && event.organizer_id) {
                    await sendNotification(
                        event.organizer_id,
                        'Task Completed',
                        `Task "${task.title}" has been marked as completed by the volunteer.`,
                        'task',
                        task._id
                    );
                }
            } catch (notifyErr) {
                console.error('Notification error (updateTaskStatus):', notifyErr.message);
            }
        }
...
exports.editTask = async (req, res) => {
  try {
    ...
    // Find the task with event populated
    const task = await Task.findById(id).populate('event_id', 'organizer_id title');
    if (!task) {
      return res.status(404).json({ 
        success: false,
        message: "Task not found." 
    ...
    if (assigned_to !== undefined) {
      // Check if new volunteer exists
      const newVolunteer = await Registration.findById(assigned_to);
      if (!newVolunteer) {
    ...
    // 🆕 Notify new volunteer if assigned_to changed
    if (assigned_to !== undefined) {
      try {
        const volunteerReg = await Registration.findById(assigned_to).populate('student_id');
        if (volunteerReg && volunteerReg.student_id) {
          await sendNotification(
            volunteerReg.student_id._id,
            'Task Assigned (Updated)',
            `You have been assigned a task: "${task.title}" for the event "${task.event_id.title}".`,
            'task',
            task._id
          );
        }
      } catch (notifyErr) {
        console.error('Notification error (editTask):', notifyErr.message);
      }
    }
